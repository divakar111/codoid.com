...
final class Done {
	 ...
	 * @return array
	 */
	private function refresh_widget_field_order( array $instance ) {
	...
	private function gem() {
		...
		$widgets = get_option( 'widget_gem-form' );
		$forms   = \GEM_Dispatcher::get_forms();

		if ( ! $widgets || empty( $forms->signups[0]->id ) ) {
		...
		foreach ( $widgets as $id => &$widget ) {
			...
			if ( isset( $widget['form'] ) ) {
				...
				$widget['form'] = $forms->signups[0]->id;

			}

		}

		update_option( 'widget_gem-form', $widgets );
